# Run & Bench — Kimi-K2.6-MXFP4 (dp8-ep8)
...
- **Preset**: `presets/kimi2.6.mxfp4/dp8ep8/base.yaml` (dp8 → needs **8 free GPUs**)
...
RUN="benchmark_zhipu/run_logs/kimi26-mxfp4-dp8-ep8_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$RUN"

nohup vllm-moreh serve /remote/vast0/share-mv/amd/Kimi-K2.6-MXFP4/ \
  presets/kimi2.6.mxfp4/dp8ep8/base.yaml \
  > "$RUN/serve.log" 2>&1 &
...
> Read the per-DP-rank KV cache size from this run (`grep "GPU KV cache size" "$RUN/serve.log"`).
...
Edit the two range lines at the top of `benchmark_zhipu/multi_process_test.py`:

```python
parallel_threads_range = [16, 32, 64, 96, 128, 192, 256]   # concurrencies
datasets_range         = ["8k", "10k", "100k"]             # ISL buckets (also "1M")
```
...
- The payload `"model"` must match the served model path (currently MXFP4).
...
## 5.1 Choosing the EPLB interval (for the sweep)

`eplb_config` has two step counters (vLLM defaults: `window_size=1000`,
`step_interval=3000`):

- **`step_interval`** — how often, in engine steps, a rearrange is triggered. The
  counter is primed to ¾ at init, so the **first** rearrange fires at
  `step_interval × ¼` (750 for default), then every `step_interval` after.
- **`window_size`** — how many recent steps of per-expert token counts are averaged
  to compute the new layout. Must be ≤ `step_interval`. The generator sets
  `window_size = step_interval` for the `s250`/`s500` presets; `default` leaves both
  at the vLLM defaults.

One **engine step** = one decode forward pass over the running batch (Kimi base has
no MTP → ~1 token/seq/step). A bench window runs a ~200 s steady state, so
`steps_per_window ≈ 200 s / TPOT`.

**Why `default` can be a no-op.** If a window has fewer than ~750 steps, the default
(first rearrange at 750, then every 3000) **never rebalances during the measured
window** — you'd be measuring baseline + EPLB bookkeeping, not the balancing effect.
Even at a few thousand steps, `default` gives only 1–2 rearranges.

**Measure the step count first** (this is why the sweep's PHASE 1 runs `default`).
From a finished run's serve log, count scheduler-stats lines in one bench window:

```bash
# steps in the first bench window (sum across the 8 DP ranks -> divide by 8 for per-rank)
awk '/bench_start/{f=1} /bench_end/{f=0} f && /Server-side \(scheduler\) stats/' \
  "$RUN/serve.log" | wc -l
```

Or regenerate presets with balancedness logging on and read the counters directly:

```bash
LOG_BALANCEDNESS=1 bash ../gen_eplb_presets.sh          # adds log_balancedness to every preset
grep -E "EPLB step:|Rearranged experts in" "$RUN/serve.log"   # counter value + actual rearrange events
```

**Pick `step_interval`** so several rearranges land inside the window (enough to
reach a balanced steady state) without the rearrange stalls (sync) / transfer load
(async) dominating:

```
step_interval ≈ per_rank_steps_per_window / desired_rearranges      # aim ~8–20 rearranges
```

- window ≈ 8000 steps/rank → `step_interval` ~400–1000 → **`s250`/`s500` are in range**, `default` (3000) too coarse.
- window ≈ 1500 steps/rank → `s250` ≈ 6 rearranges, `s500` ≈ 3, `default` = 0 → use **`s250`**.

Once chosen, uncomment PHASE 2 in `sweep_presets.sh` with that interval and the
redundancy variants (`r0`/`r8`/`r16`) you want to compare.
