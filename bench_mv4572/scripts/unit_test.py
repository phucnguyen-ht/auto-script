...
def main(parallel_threads: int, time_limit: float, ignore_time_start: float, ignore_time_end: float, data_path: str, output_path: str, log_file_name: str, port: int, encoding_size: int, cache_warmup: bool = False, warmup_only: bool = False, kv_cache_tokens: int = 0):
    ...
    # Adaptive prompt count is OFF by default: the fixed slices (e.g. data[:10] for
    # 100k) are already smaller than floor(KV/ISL)-1 on this KV, so adaptive would only
    # ADD distinct prefixes (harder to keep prefix cache ~100%). Opt in with
    # USE_ADAPTIVE_PROMPTS=1 (and a >0 --kv_cache_tokens) only when the KV is so small
    # that the fixed slice would overflow it.
    use_adaptive = os.environ.get("USE_ADAPTIVE_PROMPTS", "0").strip().lower() in ("1", "true", "yes", "on")
    if use_adaptive and kv_cache_tokens > 0 and encoding_size > 0:
        ...
        # baseline's KV) would overflow the smaller KV -> prefix thrash.
    ...
    # --- (MV-4572) SUPPRESS-WARMUP: ARM EPLB ở "gap" NÀY (sau warmup, TRƯỚC marker
    # bench_start) để cửa sổ đo bắt đầu ở trạng thái đã bật lại + counter sạch (primed ¾),
    # KHÔNG thêm overhead ở đầu bench. Server tự khởi động suppressed khi
    # VLLM_MOREH_EPLB_SUPPRESS_WARMUP=1 -> warmup không thu tải/không rearrange -> eplb_arm
    # bật lại cho bench. Server no-op nếu cơ chế/EPLB tắt (nên gọi khi =0 chỉ để test plumbing).
    # CÁCH DÙNG: (1) đặt VLLM_MOREH_EPLB_SUPPRESS_WARMUP=1 trong preset; (2) BỎ COMMENT dưới.
    # (eplb_suppress trước warmup chỉ cần khi chạy NHIỀU chu kỳ warmup+bench trong 1 server;
    #  sweep hiện tại 1 chu kỳ/server nên chỉ cần eplb_arm.)
    # try:
    #     requests.post("http://localhost:8000/collective_rpc",
    #                   json={"method": "eplb_arm"}, timeout=30)
    # except Exception as e:
    #     print(f"[eplb_arm] skipped: {e}")
    try:
