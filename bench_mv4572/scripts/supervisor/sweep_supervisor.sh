...
run_sweep() {
  while :; do
    ...
    podman exec "${CONTAINER}" bash -lc "export AITER_MOREH_ROOT_DIR='${AITER_MOREH_ROOT_DIR_VAL}'; cd '${SCRIPTS}' && SWEEP_ROOT='${CAMPAIGN}' bash sweep_presets.sh" 2>&1 | tee -a "${LOG}"
