...
def collect_eplb(port):
    """[MV-4572] Flush EPLB per-step token buffer -> per-rank .npz in out_dir.
    out_dir = EPLB_COLLECT_DIR if set, ELSE derived from REBENCH_RESULTS_DIR
    ($RUN/results -> $RUN/eplb_tokens). run_and_bench ALWAYS sets REBENCH_RESULTS_DIR,
    so this works even if EPLB_COLLECT_DIR did not propagate. Always logs; server returns
    n_samples=0 when EPLB/log_balancedness is off (base preset). To disable: pass no dir."""
    out_dir = os.environ.get("EPLB_COLLECT_DIR")
    if not out_dir:
        print("[EPLB collect] SKIP: EPLB_COLLECT_DIR not set (script phải truyền)")
        return
    try:
        url = f'http://localhost:{port}/collect_eplb'
        r = requests.post(url, json={"out_dir": out_dir}, timeout=900)
        print(f"[EPLB collect] Port {port} -> {out_dir}: HTTP {r.status_code} {r.text[:300]}")
    except requests.exceptions.RequestException as e:
        print(f"[EPLB collect] Port {port}: failed - {e}")
...
# Wrapper env overrides (manual runs: edit the two range lines above).
...
# env-overridable (wrapper points it at the run folder), else local ./results.
...
def run_warmup(port, dataset):
    ...
    output_path = f"{RESULTS_DIR}/warmup-longbenchv2-{dataset}_{time.time()}"
...
if __name__ == "__main__":
    ...
    for dataset in datasets_range:
        ...
        for parallel_threads in parallel_threads_range:
            run_warmup(port, dataset)
            run_bench(port, parallel_threads, dataset)
            # [MV-4572] collect_eplb ĐÃ CHUYỂN sang sweep_presets_profile.sh, gọi SAU
            # wait_for_traces. Gọi ở ĐÂY (ngay sau bench) va vào lúc torch-profiler đang dump
            # trace gzip -> 8 EngineCore kẹt shm_broadcast -> chỉ 1 DP engine kịp collect, 7 rank
            # còn lại timeout (900s) -> chỉ 1 file .npz. Post /collect_eplb khi profiler dump XONG
            # (engine rảnh) thì DPLB collective_rpc mới broadcast tới đủ 8 DP rank.
            # collect_eplb(port)
