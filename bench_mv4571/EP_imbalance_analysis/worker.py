...
def parse_args():
    parser = argparse.ArgumentParser(description="Simulate balanced expert parallelism")
    parser.add_argument(
        "--job-path",
        type=str,
        required=True,
        help="Path to the log file containing topk_ids and expert selection counts.",
    )
    parser.add_argument(
        "--output-path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the sample input torch file.",
    )
...
inputs = torch.load(args.sample_input_path, weights_only=False)
device = "cuda"
num_gpus = 4
...
# load jobs
logger.info(f"Loading job from log file: {args.job_path}...")
jobs = torch.load(args.job_path, weights_only=False)
real_topk_ids = jobs['topk_ids']
expert_data = jobs['expert_data']
gpu_data = jobs['gpu_data']

max_num_tokens = 0
for ids in real_topk_ids:
    max_num_tokens = max(max_num_tokens, len(ids))
logger.success(f"Max num tokens: {max_num_tokens}")
...
logger.info(f"Saving benchmark data to {args.output_path}...")
torch.save(benchmark_data, args.output_path)
logger.success("Benchmark data saved.")