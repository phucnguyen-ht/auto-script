...
#   vllm   — logs/<preset_name>/auto_readable/<ts>. run_all.sh exports
...
#   sglang — logs_sglang/auto_readable/<ts> (no preset).
if [ "${BACKEND,,}" = "sglang" ]; then
    AUTO_LOG_DIR="${SCRIPT_DIR}/logs_sglang/auto_readable"
else
    ...
    AUTO_LOG_DIR="${SCRIPT_DIR}/logs/${PRESET_NAME}/auto_readable"
fi
ts="$(date +%Y%m%d_%H%M%S)"
RUN_DIR="${AUTO_LOG_DIR}/${ts}"
mkdir -p "${RUN_DIR}"

CURL_URL="http://localhost:${SERVER_PORT}/v1/chat/completions"
