...
async def fetch_spec_decode_metrics(base_url: str) -> Optional[SpecDecodeMetrics]:
    metrics_url = f"{base_url}/metrics"
    try:
        async with _create_bench_client_session() as session:
            async with session.get(metrics_url, headers=get_auth_headers()) as response:
...
def calculate_spec_decode_stats(
    before: Optional[SpecDecodeMetrics],
    after: Optional[SpecDecodeMetrics],
    ...
    if delta_drafts > 0:
        positions = sorted(set(before.accepted_per_pos) | set(after.accepted_per_pos))
...
async def _call_profile_pd(profile_urls: List[Tuple[str, str]], mode: str) -> None:
    ...
    for worker_type, url in profile_urls:
        profile_output = await async_request_profile(api_url=url.rstrip("/") + endpoint)
...
def get_dataset(args, tokenizer, model_id=None):
    ...
    elif args.dataset_name == "jsonl-text":
        assert not tokenize_prompt
        input_requests = sample_jsonl_text_requests(
            dataset_path=args.dataset_path,
            num_requests=args.num_prompts,
            tokenizer=tokenizer,
            output_len=args.jsonl_output_len,
            context_len=args.jsonl_context_len,
            start_index=args.jsonl_start_index,
            text_field=args.jsonl_text_field,
            tokens_field=args.jsonl_tokens_field,
            apply_chat_template=args.apply_chat_template,
        )
...
def sample_jsonl_text_requests(
    dataset_path: str,
    num_requests: int,
    ...
    if not dataset_path:
        raise ValueError("--dataset-path is required for jsonl-text dataset")

    dataset: List[DatasetRow] = []
    skipped = 0
    used_token_metadata = 0

    end_index = start_index + num_requests
    ...
    with open(dataset_path, encoding="utf-8") as f:
        for line_no, line in enumerate(f, start=1):
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid JSONL record at {dataset_path}:{line_no}: {exc}"
                ) from exc

            if text_field not in data:
                raise ValueError(
                    f"Missing text field '{text_field}' at {dataset_path}:{line_no}"
                )

            prompt = data[text_field]
            if not isinstance(prompt, str):
                raise ValueError(
                    f"Field '{text_field}' must be a string at {dataset_path}:{line_no}"
                )
            ...
            prompt_len = None
            if not apply_chat_template and tokens_field:
                token_metadata = data.get(tokens_field)
                if isinstance(token_metadata, int) and token_metadata > 0:
                    prompt_len = token_metadata
                    used_token_metadata += 1

            if prompt_len is None:
                if tokenizer is None:
                    raise ValueError(
                        f"Field '{tokens_field}' must be a positive integer at "
                        f"{dataset_path}:{line_no} when tokenizer loading is skipped"
                    )
            ...
            if len(dataset) == end_index:
                break
    if len(dataset) < end_index:
        for i in range(len(dataset), end_index):
            dataset.append(dataset[i%len(dataset)])
    filtered_dataset = dataset[start_index:end_index]

    print(f"Loaded {len(dataset)} jsonl-text prompts from {dataset_path}")
    print(
        f"Selected jsonl-text prompt indexes [{start_index}, {end_index}) "
        f"({len(filtered_dataset)} prompts)"
    )
    print(f"Using token metadata for {used_token_metadata} prompts")
    if skipped:
        print(f"Skipped {skipped} jsonl-text prompts")
    print(f"#Input tokens: {np.sum([x.prompt_len for x in filtered_dataset])}")
    print(f"#Output tokens: {np.sum([x.output_len for x in filtered_dataset])}")
    return filtered_dataset
...
async def get_request(
    ...
    if use_trace_timestamps:
        ...
        for request in input_requests:
            ...
            yield request
            # yield random.choice(input_requests)
    else:
...
def calculate_metrics(
    ...
    return metrics, output_lens, retokenized_output_lens
...
async def benchmark(
    ...
    if profile and pd_separated:
        ...
        if not pd_profile_urls:
            print(
                "PD separated profile URLs were not provided. "
                "Using base URL profiler endpoint."
            )

    # Start profiler
    if profile:
        if pd_separated and pd_profile_urls:
    ...
    spec_decode_metrics_before = await fetch_spec_decode_metrics(base_url)
    ...
    spec_decode_metrics_after = await fetch_spec_decode_metrics(base_url)
    spec_decode_stats = calculate_spec_decode_stats(
        spec_decode_metrics_before, spec_decode_metrics_after
    )
    ...
    metrics, output_lens, retokenized_output_lens = calculate_metrics(
    ...
    print("{:<40} {:<10.2f}".format("mean retokenized_output_lens:", sum(retokenized_output_lens) / len(retokenized_output_lens)))
    print("{:<40} {:<10.2f}".format("max retokenized_output_lens:", max(retokenized_output_lens)))
    if spec_decode_stats is not None:
        ...
        per_pos_rates = spec_decode_stats.get("per_position_acceptance_rates", [])
        if per_pos_rates:
            print("Per-position acceptance (%):")
            for i, rate in enumerate(per_pos_rates):
                print("{:<40} {:<10.2f}".format(f"  Position {i}:", rate * 100))
    print("=" * 50)

    # dump retokenized_output_lens
    # import datetime
    # with open(f"retokenized_output_lens_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt", "w") as f:
    #     for i in retokenized_output_lens:
    #         f.write(str(i) + "\n")
    ...
    if (
        ...
        if spec_decode_stats is not None:
            result["spec_decode_acceptance_rate"] = spec_decode_stats[
                "acceptance_rate"
            ]
...
def can_skip_tokenizer(args_: argparse.Namespace) -> bool:
    return False
    return (
        args_.dataset_name == "jsonl-text"
        and not args_.apply_chat_template
        and bool(args_.jsonl_tokens_field)
    )
...
def run_benchmark(args_: argparse.Namespace):
    ...
    skip_tokenizer = can_skip_tokenizer(args)

    if not skip_tokenizer and not check_chat_template(args.model):
        print(
    ...
    if skip_tokenizer:
        tokenizer = None
        print(
            "Skipping tokenizer load for jsonl-text dataset because prompt token "
            f"counts are read from '{args.jsonl_tokens_field}'."
        )
    else:
    ...
    if args.num_samples_per_prompt > 1:
        # Expand input_requests to accommodate num_samples_per_prompt
        expanded_requests = []
        for request in input_requests:
            for _ in range(args.num_samples_per_prompt):
                expanded_requests.append(request)
        input_requests = expanded_requests
...
if __name__ == "__main__":
    ...
    jsonl_group = parser.add_argument_group("jsonl-text dataset arguments")
    jsonl_group.add_argument(
        "--jsonl-output-len",
        type=int,
        default=256,
        help="Output length for each request in the jsonl-text dataset.",
    )
    jsonl_group.add_argument(
        "--jsonl-context-len",
        type=int,
        default=None,
        help="Drop jsonl-text requests whose prompt plus output length exceeds this context length.",
    )
    jsonl_group.add_argument(
        "--jsonl-start-index",
        type=int,
        default=0,
        help="Zero-based jsonl-text prompt index to start from before taking --num-prompts.",
    )
    jsonl_group.add_argument(
        "--jsonl-text-field",
        type=str,
    ...
    jsonl_group.add_argument(
        ...
        type=str,
        default="tokens",
        help="JSONL field containing the precomputed prompt token count. Set to empty to retokenize prompts.",
    )
    parser.add_argument(
        "--random-input-len",
        type=int,
    ...
    # In PD separated mode, prefill and decode workers expose their own
    # profiler endpoints. Accept both lists so a single run can profile all
    # workers around the same request window.
    parser.add_argument(
        "--profile-prefill-url",
        type=str,
        nargs="*",
    ...
    # for RL generation benchmarking
    parser.add_argument(
        "--num-samples-per-prompt",
        type=int,
        default=1,
        help="Number of samples to generate per prompt. Default is 1.",
    )
    parser.add_argument(
